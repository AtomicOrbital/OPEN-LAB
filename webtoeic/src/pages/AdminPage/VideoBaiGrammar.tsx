...
import {
    GrammarWithVideo,
    GrammarWithVideoResponse,
    VideoBaiGrammarResponse,
    addVideoToGrammar,
    deleteVideoFromGrammar,
    fetchAllGrammarWithVideo,
    updateVideoInGrammar
} from '../../redux/VideoGrammar/VideoGrammarReducer';
...
const VideoBaiGrammar: React.FC = () => {
    const allVideoBaiGrammar = useSelector((state: RootState) => state.VideoGrammarReducer.grammarWithVideos) as GrammarWithVideoResponse[] | null;


    const dispatch: DispatchType = useDispatch();
    console.log("allVideoBaiGrammar", allVideoBaiGrammar);
    useEffect(() => {
        dispatch(fetchAllGrammarWithVideo());
    }, [dispatch])

    const initializeVideoFormData = (baiGrammarId?: number): VideoFormData => ({
        videoId: null,
        title: "",
        description: "",
        baiGrammarId: baiGrammarId ?? 0
    });
    ...
    const [editVideoData, setEditVideoData] = useState<VideoFormData>(initializeVideoFormData());
    ...
    const [videoFormData, setVideoFormData] = useState<VideoFormData>(initializeVideoFormData());
    ...
    const handlePanelChange = (activeKey: string | string[]) => {
        ...
        const newBaiGrammarId = Array.isArray(activeKey) ? Number(activeKey[0]) : Number(activeKey);
        console.log("Selected baiGrammarId", newBaiGrammarId);
        setVideoFormData(initializeVideoFormData(newBaiGrammarId));
    };
    ...
    const handleSubmitVideo = async () => {
        ...
        const data = {
            ...
            baiGrammarId: videoFormData.baiGrammarId
        };

        console.log("data", data);

        const response = await dispatch(addVideoToGrammar(data));
    ...
    const showEditModal = (videoItem: VideoBaiGrammarResponse, baiGrammarId: number) => {
        setEditVideoData({
            videoId: videoItem.id,
            title: videoItem.title,
            description: videoItem.description,
            baiGrammarId: baiGrammarId,
            file: undefined
        });
        console.log("baiGrammarId", videoItem);
    ...
    const handleEditSubmit = async () => {
        // Kiểm tra xem editVideoData có videoId hợp lệ không
        if (editVideoData.videoId != null) {
            // Gọi hàm xử lý việc cập nhật video
            await handleSubmitEditVideo(editVideoData);
            // Đóng modal sau khi gửi dữ liệu
            setEditModalVisible(false);
        } else {
            // Xử lý trường hợp không có videoId
            console.error("No videoId provided for editing");
        }
    };



    const handleSubmitEditVideo = async (videoData: VideoFormData) => {
        ...
        try {
            if (videoData.videoId != null) {
                await dispatch(updateVideoInGrammar({
                    file: videoData.file?.file,
                    title: videoData.title,
                    description: videoData.description,
                    videoId: videoData.videoId,
                    baiGrammarId: videoData.baiGrammarId,
                })).unwrap();
    ...
    const handleDeleteVideo = async (videoId: number, baiGrammarId: number) => {
        await dispatch(deleteVideoFromGrammar({ videoId, baiGrammarId })).unwrap();
        try {
            notification.success({
    ...
    const uploadProps = {
        ...
        customRequest: async (options: any) => {
            ...
            try {
                await http.post("/api/baigrammar/create", formData);
                options.onSuccess();
            } catch (error) {
    ...
    return (
        <Fragment>
            <Collapse className={styles.jittery} onChange={handlePanelChange}>
                {allVideoBaiGrammar?.map((grammarWithVideo: GrammarWithVideoResponse) => (
                    <Panel header={grammarWithVideo.grammarWithVideoDTO.tenBaiGrammar} key={grammarWithVideo.grammarWithVideoDTO.baiGrammarId}>
                        <Form layout="vertical" onFinish={handleSubmitVideo}>
                            ...
                            <Form.Item style={{ display: 'none' }}>
                                <Input name="baiGrammarId" value={videoFormData.baiGrammarId} onChange={handleInputChange} />
                            </Form.Item>
                        ...
                        <List
                            dataSource={grammarWithVideo.videos}
                            renderItem={(videoItem: VideoBaiGrammarResponse) => {
                                const baiGrammarId = grammarWithVideo.grammarWithVideoDTO.baiGrammarId;
                                return (
                                    <List.Item style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
                                            <Button
                                                type='primary'
                                                style={{ marginRight: '5px' }} onClick={() => showEditModal(videoItem, baiGrammarId)}>
                                                <EditOutlined />
                                            </Button>
                                            <Popconfirm
                                                title="Are you sure you want to delete this entry?"
                                                onConfirm={() => handleDeleteVideo(videoItem.id, grammarWithVideo.grammarWithVideoDTO.baiGrammarId)}
            ...
            >
                ...
                >
                    ...
                    <Form.Item name="baiGrammarId" style={{ display: 'none' }}>
                        <Input name="baiGrammarId" hidden />
                    </Form.Item>
...
export default VideoBaiGrammar;
